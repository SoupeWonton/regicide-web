...
Not yet built: the four-axis camp allocation (♦/♥/♠/♣ pips) and the Forge (now scoped as the **token** system above — +permanent value/suit tokens on cards). **Exile / deck-thinning is retired by design (NO-EXILE rule — the deck only grows).** See "Known gaps" below.
...
Not yet implemented: the **four-axis camp allocation** (spend pips across ♦ draw / ♥ recover / ♠ shield / ♣ edge), the **class tree** (post-signature progression), specialization effects, meta currency, multiple map variants per chapter, and mid-session reconnection. (The **Forge** is now built as the ascending-deck token system — see the build status above.) The **party** economy is deliberately left on the old single-relic assumptions for now — the redesign is solo-first.

Removed (do **not** reintroduce): **memories** (the old `memory_draft` chapter-end draft), **preparations** (camp-activated `p-*` items), and **all card-removal / exile** — the **NO-EXILE rule** (2026-06-16): no mechanic may thin the deck; **the deck only ever grows** (recruit numbers + royals), and each continent starts from a complete deck. The Sanctum Exile rite, the Exile-class camp ability, the "Tithe of the Severed" siege ult, and the `exile_pick` flow are all gone — do **not** re-add deck thinning (incl. "Exile-as-a-landmark-service"). The **Exile class** keeps its roster slot + signature Transmute tokens but is **parked** (no active ability) pending a non-thinning repurpose. Memories/preparations are gone from types, content, engine, and UI.
