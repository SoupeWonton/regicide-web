...
questions: [Q5]
topics: [class-personality]
---

# Class progression — enabler passives × payoff ladders (for pairing)

**Status:** Showcase for a Landry↔Gab session. Two **orthogonal** menus per class — a pool of
**Passive enablers** and a pool of **Linear payoff ladders** — meant to be **paired** live
(e.g. *Ironclad × Counter*). Over-generated; cut hard. Names/numbers illustrative — **the Rule is
the thing to vote on.** Feeds **Q5**.
...
|---|---|---|---|
| **A · Fortress** *(defense → cards)* | Fully block (net 0) → draw 1. | Draw **1 per 5 excess shield** beyond the attack. | Start of turn, draw **1 per 5 shield you hold** — defense is a deck engine. |
| **B · Thornline** *(defense → damage)* | Fully block → deal **3**. | Fully block → deal **half your shield value**. | **Every point of Spade shield is also a point of damage** to the enemy. |
