...
- **No new combat rules.** The tutorial uses only existing engine verbs (suits, combos,
  Ace pairing, immunity, Jester, exact-kill recruit). It is *scripting on top of the
...
*See also:* `docs/design/ascending-deck.md` (exact-kill recruit, the deck model),
`docs/design/campaign/campaign-bible.md` (combat canon).
